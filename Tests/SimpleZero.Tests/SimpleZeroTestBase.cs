...
using SimpleZero.Migrations.SeedData;
...
namespace SimpleZero.Tests
{
    public abstract class SimpleZeroTestBase : AbpIntegratedTestBase<SimpleZeroTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected SimpleZeroTestBase()
        {
        ...
        protected void UsingDbContext(Action<SimpleZeroDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SimpleZeroDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SimpleZeroDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SimpleZeroDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SimpleZeroDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SimpleZeroDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SimpleZeroDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SimpleZeroDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SimpleZeroDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SimpleZeroDbContext, Task<T>> func)
        {
            T result;
